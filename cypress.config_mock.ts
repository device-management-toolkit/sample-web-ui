/*********************************************************************
 * Copyright (c) Intel Corporation 2022
 * SPDX-License-Identifier: Apache-2.0
 **********************************************************************/

import { defineConfig } from 'cypress'
import { readCertificateDomain, findCertificateByDomain } from './cypress/support/certTasks'


export default defineConfig({
  reporter: 'junit',
  reporterOptions: {
    mochaFile: 'test-reports/cypress-ui-test-output-[hash].xml'
  },
  viewportWidth: 1920,
  viewportHeight: 1080,
  projectId: 'mxeztq',
  env: {
    BASEURL: 'http://localhost:4200/',
    ISOLATE: 'Y',
    FQDN: '192.168.8.50',
    ACTIVATION_URL: '192.168.8.50',
    PROFILE_NAME: 'test-profile-cira',
    MPS_USERNAME: 'standalone',
    MPS_PASSWORD: 'G@ppm0ym',
    AMT_PASSWORD: 'P@ssw0rd',
    MEBX_PASSWORD: 'P@ssw0rd',
    WIFI_SSID: 'test',
    WIFI_PSK_PASSPHRASE: 'P@ssw0rd',
    VAULT_ADDRESS: 'http://localhost:8200',
    VAULT_TOKEN: 'myroot',
    TEST_SUITE: 'sequential', // 'quick', 'smoke', 'sequential'
    TEST_SEQUENCE: 'ordered', // 'parallel', 'ordered'
    PROVISIONING_CERT:
      '',
    PROVISIONING_CERT_PASSWORD: 'P@ssw0rd',
    DOMAIN_SUFFIX: 'vprodemo.com',
    PROXY_SERVER: '',//'proxy-chain.intel.com',  // Your Intel proxy hostname
    PROXY_PORT: '',//'911', // Your proxy port
    RPC_DOCKER_IMAGE: 'vprodemo.azurecr.io/rpc-go:latest',
    RPC_GO_VERSION: 'v2.48.8'
  },
  chromeWebSecurity: false,
  e2e: {
    experimentalStudio: true,
    screenshotOnRunFailure: false,
    setupNodeEvents(on, config) {
      // Add file existence task
      on('task', {
        fileExists(filename: string) {
          const fs = require('fs')
          return fs.existsSync(filename)
         },
        log(message: string) {
          console.log(message)
          return null
        },
        readCertificateDomain,
        findCertificateByDomain
      })

      on('before:browser:launch', (browser, launchOptions) => {
        if (browser.name === 'chrome') {
          launchOptions.args.push('--ignore-ssl-errors=yes')
          launchOptions.args.push('--ignore-certificate-errors')
          launchOptions.args.push('--ignore-ssl-errors-list')
          launchOptions.args.push('--ignore-certificate-errors-spki-list')
          launchOptions.args.push('--disable-web-security')
          launchOptions.args.push('--allow-running-insecure-content')
        }
        return launchOptions
      })
    },
    specPattern: [
      'cypress/e2e/01-prerequisite/**/*.spec.ts',
      'cypress/e2e/02-authentication/**/*.spec.ts',
      'cypress/e2e/03-ieee8021x/**/*.spec.ts',
      'cypress/e2e/04-wireless/**/*.spec.ts',
      'cypress/e2e/05-cira/**/*.spec.ts',
      'cypress/e2e/06-proxy-config/**/*.spec.ts',
      'cypress/e2e/07-profiles/**/*.spec.ts',
      'cypress/e2e/08-domains/**/*.spec.ts',
      'cypress/e2e/09-rpc/**/*.spec.ts',
      'cypress/e2e/10-devices/**/*.spec.ts',
      'cypress/e2e/11-eventlogs/**/*.spec.ts',
      'cypress/e2e/12-dashboard/**/*.spec.ts'
    ]
  }
})
