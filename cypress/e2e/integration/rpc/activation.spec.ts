...
export const getAmtInfo = (infoCommand: string, config: Cypress.ExecOptions = execConfig): Cypress.Chainable<AMTInfo> => {
...
export const buildActivateCommand = (opts: ActivateCommandOptions): string => {
  ...
  const profileDir = opts.profileYamlFile ? opts.profileYamlFile.substring(0, opts.profileYamlFile.lastIndexOf('/')) : ''
  const profileFileName = opts.profileYamlFile ? opts.profileYamlFile.substring(opts.profileYamlFile.lastIndexOf('/') + 1) : ''
...
export const buildDeactivateCommand = (opts: DeactivateCommandOptions): string => {
  ...
  if (isCloud) {
    const args = `${cmd} -u wss://${opts.fqdn}/activate -n ${common_flag}`
    return buildRpcCommand({ isWin: opts.isWin, rpcDockerImage: opts.rpcDockerImage }, 'rpc.exe', args)
  }

  const args = `${cmd} --local --skip-amt-cert-check ${common_flag}`
  return buildRpcCommand(
    { isWin: opts.isWin, rpcDockerImage: opts.rpcDockerImage },
    'rpc.exe',
    args
  )
}
