...
describe('KVM Component E2E Tests', () => {
  ...
  beforeEach(() => {
    ...
    // Mock redirection token API (needed for all KVM tests)
  ...
  describe('KVM Initialization Flow', () => {
    ...
    it('should prevent duplicate API calls during initialization', () => {
      ...
      cy.myIntercept('GET', `**/api/v1/amt/kvm/displays/${deviceId}`, (req) => {
        displayCallCount++
        req.reply({
          statusCode: httpCodes.SUCCESS,
          body: kvm.displaySelection.success.response
      ...
      cy.myIntercept('GET', `**/api/v1/amt/power/state/${deviceId}`, (req) => {
        powerCallCount++
        req.reply({
          statusCode: httpCodes.SUCCESS,
          body: kvm.powerState.poweredOn.response
