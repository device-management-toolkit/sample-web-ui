  ...
  before(function () {
    ...
    cy.request({
      ...
      cy.request({
        ...
        if (!connected.length) {
          cy.task('log', '  ⚠️  No connected AMT devices found — skipping')
          this.skip()
          return
        }
        resolvedDeviceGuid = connected[0].guid
        cy.task('log', `  ✅ Auto-detected GUID (first connected): ${resolvedDeviceGuid}`)
      })
    })
  })
