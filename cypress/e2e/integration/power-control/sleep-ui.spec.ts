...
/**
 * Isolated run: TC_POWER_ACTION_SLEEP via Web UI dropdown only.
 *
 * Flow:
 ...
 *   4. Click "Sleep"
 *   5. Wait 1 minute for the OS to process the ACPI signal
 *   6. Verify AMT power state via GET /api/v1/amt/power/state/:guid
 *   7. Wake device back to On via POST OsToFullPower (action 500)
 *
...
import { PowerStateValues, PowerStateLabels } from 'cypress/e2e/fixtures/api/power'
import {
  UiTestContext,
  webUiBaseUrl,
  mpsBaseUrl,
...
describe('TC_POWER_ACTION_SLEEP - Sleep via Web UI dropdown (isolated run)', () => {
  // -- before: authenticate via MPS API + resolve GUID -------------------
  before(function () {
    cy.task('log', '\n═══════════════════════════════════════════════════════')
    cy.task('log', ' TC_POWER_ACTION_SLEEP ─ Web UI isolated run')
