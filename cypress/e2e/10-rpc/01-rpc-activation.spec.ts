  ...
  describe('Activation', () => {
    ...
    beforeEach(() => {
      ...
      cy.exec(infoCommand, execConfig).then((result) => {
        ...
        // Check if stderr contains device info or error messages
        if (!result.stderr || result.stderr.includes('No Intel AMT') || result.stderr.includes('Failed') || result.stderr.includes('Error') || !result.stderr.includes('"amt"')) {
    ...
    it('should NOT deactivate device - invalid password', () => {
      if (!amtInfo) {
        cy.log('Skipping test - no Intel AMT device detected')
        return
      }
