...
async function fetchAmtFirmwareFromAPI(env, isCloud, apiUrl) {
  ...
  const username = isCloud
    ? (env.MPS_USERNAME || '')
    : (env.CONSOLE_USERNAME || env.MPS_USERNAME || '')
  const password = isCloud
    ? (env.MPS_PASSWORD || '')
    : (env.CONSOLE_PASSWORD || env.MPS_PASSWORD || '')
  const credsHint = isCloud
  ...
  const authorizeUrl = isCloud
    ? `${baseUrl}/login/api/v1/authorize`
  ...
  if (!token && isCloud) {
    return { firmware: null, note: 'could not authenticate to MPS API', provisioningMode: null, provisioningModeNote: 'could not authenticate to MPS API' }
  }
  ...
  if (devices.length === 0) return { firmware: null, note: 'no devices found in API', provisioningMode: null, provisioningModeNote: 'no devices found in API' }
  ...
  if (isCloud) {
    ...
    const firmware = (device && device.deviceInfo && device.deviceInfo.fwVersion)
      ? String(device.deviceInfo.fwVersion)
      : null
    ...
    if (device && device.guid) {
      ...
      if (vr.ok && vr.data) {
        ...
        pmNote = pm
          ...
          : pmRaw !== undefined ? `unrecognized ProvisioningMode value: ${pmRaw}` : 'ProvisioningMode not in response'
      } else {
    ...
    if (!device || !device.guid) return { firmware: null, note: 'no device GUID available from Console API', provisioningMode: null, provisioningModeNote: 'no device GUID available' }

    const amtVerUrl = `${baseUrl}/api/v1/amt/version/${device.guid}`
    const vr = await fetchJSON(amtVerUrl, 8000, { Authorization: `Bearer ${token}` })
    if (!vr.ok || !vr.data) return { firmware: null, note: `could not fetch AMT version for ${device.guid}`, provisioningMode: null, provisioningModeNote: `could not fetch AMT version for ${device.guid}` }
    ...
    const pmNote = pm
      ...
      : pmRaw !== undefined ? `unrecognized mode value: ${pmRaw}` : 'ProvisioningMode not in response'
    ...
    return { firmware: null, note: 'AMT version entry not found in Console API response', provisioningMode: pm, provisioningModeNote: pmNote }
  }
}
...
async function fetchVersionInfo(config) {
  ...
  const authorizeUrl = isCloud
    ? `${mpsServer}/login/api/v1/authorize`
    : `${mpsServer}/api/v1/authorize`
  const apiToken = (isCloud && env.MPS_USERNAME && env.MPS_PASSWORD)
  ...
  //
  // Three keys support live API fetching when the value is empty, undefined, or FETCH_FROM_API:
  ...
  // In CI, prefer "FETCH_FROM_API" over "" — Cypress strips empty CYPRESS_* env vars.
  ...
  const cvJson = env.COMPONENT_VERSIONS
  ...
  for (const key of orderedVersionKeys) {
    ...
    const shouldFetch = !val || val === FETCH_FROM_API
    if (!shouldFetch) {
...
module.exports = { fetchVersionInfo, HIDE_MARKER, FETCH_FROM_API }
