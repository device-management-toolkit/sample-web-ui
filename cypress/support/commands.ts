...
Cypress.Commands.add('enterProfileInfoV2', (formData: any) => {
  ...
  if (formData.wifiConfigs != null && formData.wifiConfigs.length > 0) {
    // Wait for WiFi autocomplete to be available (requires wireless configs to load from server)
    // Use should('exist') with timeout to handle async loading in real API mode
    cy.get('body').then(($body) => {
      if ($body.find('[data-cy=wifiAutocomplete]').length > 0) {
        ...
        cy.log('WiFi autocomplete not visible - wireless configs may not be available on server')
      }
    })
  }
})
...
Cypress.Commands.add('enterIEEE8021xInfo', (config: IEEE8021xConfig) => {
  ...
  // Only fill pxeTimeout for wired configs (field is hidden for wireless)
  if (config.pxeTimeout != null && config.wiredInterface) {
